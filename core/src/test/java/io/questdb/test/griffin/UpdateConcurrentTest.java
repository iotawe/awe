        ...
        assertMemoryLeak(() -> {
            ...
            for (int k = 0; k < numOfWriters; k++) {
                Thread writer = new Thread(() -> {
                    try {
                        final SqlCompilerImpl updateCompiler = new SqlCompilerImpl(engine, null, snapshotAgent);
            ...
            for (int k = 0; k < numOfReaders; k++) {
                Thread reader = new Thread(() -> {
                    ...
                    try {
                        final SqlCompilerImpl readerCompiler = new SqlCompilerImpl(engine, null, snapshotAgent);
                        barrier.await();
